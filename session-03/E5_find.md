## Aufgaben zu find (Pt 1/2)

Das Wort "Einträge" meint hier "Dateien" oder "Verzeichnisse",
"Datei" meint einen "Eintrag" vom Typ "f",
Verzeichnis meint einen Eintrag vom Typ "d"

Vorbereitung: Dein Standort sollte Dein `$HOME`-Verzeichnis sein.
...
Vorbereitung: Lege ein Verzeichnis `roemische-lager/SEISDRUM` an (beachte die GROSSSCHREIBUNG!!)
...
## Aufgaben zu find (Pt 2/2)

Alle timestamps einer datei seht ihr mit dem Kommando

```
stat ../some-file.txt
```
Vorbereitung: starte das Kommando `sudo /scripts/start-logging.sh`
